...
RSpec.describe Services::WorkerTasks::RequestRemoteParseGoalsFill do
  ...
  it 'do nothing if no required genres exists' do
    Services::WorkerTasks::RequestRemoteParseGoalsFill.new.call
  ...
  describe 'respond state to caller' do
    it 'return false when execution failed' do
      expect(Services::WorkerTasks::RequestRemoteParseGoalsFill.new.call).to eq false
    end

    it 'return true when execution successful' do
      create_genres
      expect(Services::WorkerTasks::RequestRemoteParseGoalsFill.new.call).to eq true
    end
  end
end
